...
import { ChevronLeft, ChevronRight, Search, Filter, Calendar, FileDown } from 'lucide-react';
...
export default function ReportsPage() {
  ...
  const subtitleDateText = `Dữ liệu tổng hợp từ ${formatDateString(dateRange.start)} đến ${formatDateString(dateRange.end)}`;
  ...
  return (
    <div className="reports-page px-2 md:px-4 py-4">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 mb-6">
        <div>
          <h2 className="text-2xl font-bold font-headline tracking-tight text-slate-900">
            Báo cáo Nhập - Xuất - Tồn chi tiết
          </h2>
          <p className="text-xs text-slate-500 mt-1">{subtitleDateText}</p>
        </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4 bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
        <div className="relative flex-1 max-w-md">
          <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
          <input
            type="text"
            placeholder="Tìm kiếm theo mã hoặc tên linh kiện..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-full pl-9 pr-4 py-1.5 border border-slate-200 rounded-lg text-xs focus:border-primary focus:ring-2 focus:ring-primary/10 outline-none text-slate-700"
          />
        </div>
            ...
            <table className="w-full text-left border-collapse text-slate-700">
              ...
              <tfoot>
                <tr className="bg-slate-100 border-t-2 border-primary/20 text-slate-900">
                  <td className="font-bold uppercase tracking-wider text-[10px] py-3 pl-4" colSpan="2">
                    Tổng cộng cuối kỳ
                  </td>
                  {/* Totals */}
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 py-2 mb-6">
          <div className="flex items-center gap-6">
            ...
            <p className="text-[11px] text-slate-500 font-medium italic">
              Đang hiển thị {reportData.length} trên tổng số {totalCount} linh kiện trong kho.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex items-start justify-between hover:shadow-md transition-shadow">
          <div>
            ...
            <h3 className="text-2xl font-black text-slate-900">{formatNumber(dashboardStats.totalStock)}</h3>
            <p className={`text-[10px] font-bold flex items-center gap-1 mt-2 ${
              Number(dashboardStats.stockPercentChange) >= 0 ? 'text-green-600' : 'text-red-500'
            }`}>
              {Number(dashboardStats.stockPercentChange) >= 0 ? '+' : ''}
              {dashboardStats.stockPercentChange}% so với tháng trước
            </p>
          </div>
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex items-start justify-between hover:shadow-md transition-shadow">
          <div>
            ...
            <h3
              className="text-2xl font-black text-slate-900 cursor-help"
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex items-start justify-between hover:shadow-md transition-shadow">
          <div>
            ...
            <h3 className="text-2xl font-black text-slate-900">{dashboardStats.importCount}</h3>
            <p className="text-[10px] text-slate-500 font-medium flex items-center gap-1 mt-2">
              Trung bình {dashboardStats.averageImportsPerDay} phiếu/ngày
            </p>
          </div>
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex items-start justify-between hover:shadow-md transition-shadow">
          <div>
            ...
            <h3 className={`text-2xl font-black ${dashboardStats.lowStockCount > 0 ? 'text-red-500' : 'text-slate-900'}`}>
              {dashboardStats.lowStockCount}
            </h3>
            <p className={`text-[10px] font-bold flex items-center gap-1 mt-2 ${
              dashboardStats.lowStockCount > 0 ? 'text-red-500' : 'text-slate-500'
            }`}>
              {dashboardStats.lowStockCount > 0 ? 'Cần nhập hàng ngay' : 'Tồn kho an toàn'}
            </p>
          </div>
          <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-lg font-bold ${
            dashboardStats.lowStockCount > 0 ? 'bg-red-50 text-red-500' : 'bg-slate-50 text-slate-500'
          }`}>
            ⚠️
          </div>
        </div>

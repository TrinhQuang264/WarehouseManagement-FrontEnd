...
export default function CustomerDetailPage({ customer, history = [], onEdit }) {
  const initials = String(customer.fullName || "KH")
    .split(" ")
    .map((part) => part[0])
    .join("")
    .slice(0, 2)
          ...
          <nav className="flex items-center justify-between text-sm text-slate-500 mb-2">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-lg bg-primary px-5 py-2.5 text-sm font-bold text-white shadow-lg shadow-primary/20 hover:opacity-90"
              >
                <Pencil size={16} />
                Chỉnh sửa thông tin
              </button>
      ...
      <div className="grid grid-cols-12 gap-6">
        <div className="col-span-12 lg:col-span-4 flex flex-col gap-6">
          <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
            <div className="mx-auto mb-6 flex h-24 w-24 items-center justify-center rounded-2xl bg-primary/10 text-3xl font-black text-primary ring-4 ring-slate-50">
              {initials}
            </div>
            <div className="text-center">
              ...
              <p className="mt-2 inline-flex rounded-full border border-primary/10 bg-primary/5 px-3 py-1 text-sm font-medium text-primary">
                {customer.code}
              </p>
            </div>

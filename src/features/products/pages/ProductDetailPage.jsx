...
function ProductImage({ imageUrl, alt, className }) {
  ...
  if (!fullImageUrl) {
    return (
      <div
        className={`flex items-center justify-center text-sm font-medium text-slate-400 ${className}`}
      >
        Không có ảnh
      </div>
    );
...
export default function ProductDetailPage() {
      ...
      try {
        ...
        if (isMounted && productData) {
          ...
          const category = allCategories.find(
            (c) => Number(c.id) === Number(productData.categoryId),
          );
          const categoryName =
            category?.name ||
            productData.categoryName ||
            `Danh mục ${productData.categoryId}`;

          setProduct({
            ...productData,
            categories: productData.categoryId ? [categoryName] : [],
            importPrice: productData.originalPrice ?? 1,
          ...
          const thumbnail = verifiedImgList.find(
  ...
  const profitMargin = (
    ((product.price - product.importPrice) / product.importPrice) *
    100
  ).toFixed(1);
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-slate-100">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-y-6 gap-x-8">
              ...
              <div className="md:row-span-2 bg-emerald-50/50 rounded-xl p-4 border border-emerald-100 flex flex-col justify-center items-center text-center">
                ...
                <div className="mt-2 text-xs font-medium bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full">
                  Sản phẩm có sẵn
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-sm font-bold text-slate-900">
                  {product.warehouseLocation || "Chưa xác định"}
                </p>
              </div>
              <div className="col-span-1 md:col-span-2">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {product.categories?.map((cat, idx) => (
                    <span
                      key={idx}
                      className="px-3 py-1 bg-white border border-slate-200 rounded-lg text-xs font-medium text-slate-700 shadow-sm"
                    >
                      {cat}
                    </span>
                  ))}
                    ...
                    {inventoryHistory.map((entry) => (
                      <tr key={entry.id} className="table-row-hover">
                        ...
                        <td className="px-4 py-4">
                          <div className="flex items-center gap-2">
                            <span
                              className={`w-2 h-2 rounded-full ${entry.type === "import" ? "bg-emerald-500" : "bg-red-500"}`}
                            />

...
export default function ImportsPage() {
  ...
  const isDetailMode =
    location.pathname.startsWith(IMPORT_URLS.list) &&
    !isAddMode &&
    !isEditMode &&
    location.pathname !== IMPORT_URLS.list;
        ...
        render: () => (
          <div className="imports-detail-actions">
            <Button
              variant="secondary"
              icon={<Edit3 size={18} />}
              onClick={() => navigate(IMPORT_URLS.edit(sourceReceipt.id))}
            >
              Chỉnh sửa
            </Button>
            <Button
  ...
  const openEdit = useCallback(
    (receipt) => navigate(IMPORT_URLS.edit(receipt.id)),
    [navigate],
  );
  const openDetail = useCallback(
    (receipt) => navigate(IMPORT_URLS.detail(receipt.id)),
    [navigate],
  );
